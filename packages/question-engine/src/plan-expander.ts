...
import type { ArticleMetadata, QuestionEngineOptions, ReadingPlanPart, TaskTemplate } from './types'
...
export async function expandReadingPlan(input: PlanExpanderInput, options?: QuestionEngineOptions) {
  if (!input.text?.trim()) {
  ...
  try {
    ...
    const _snippet = raw.slice(0, 400)
