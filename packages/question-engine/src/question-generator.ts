...
function resolveTaskPool(archetype: string, provided?: TaskTemplate[] | null): TaskTemplate[] {
...
export async function runHookWorkflow(
  ...
  const hookQuestions = await generateHookQuestions({ metadata, articleText }, options)
...
export async function runInstructionWorkflow(
  ...
  const readingPlan = await generateReadingPlan({ metadata, taskPool }, options)
