...
import { ensureArticleAnalysis, ensureArticleContent } from '@/lib/workflows/article-content'
...
async function claimNextInstructionQuiz(): Promise<QuizRow | null> {
  const { data, error } = await supabase.rpc('claim_next_instruction_job').maybeSingle()
...
async function handleInstructionJob(quiz: QuizRow): Promise<ProcessResult> {
  ...
  try {
    ...
    const analysis = await ensureArticleAnalysis(prepared.article, prepared.content)
...
async function handleHookJob(quiz: QuizRow): Promise<void> {
  ...
  } else if (refreshedQuiz.status === 'failed' || refreshedQuiz.status === 'not_required') {
    await supabase
      .from('quizzes')
...
export async function processNextPendingQuiz(): Promise<ProcessResult> {
  ...
  logger.info({ quizId: instructionQuiz.id }, 'Processing next instruction workflow')
  return handleInstructionJob(instructionQuiz)
}

export async function processQuizById(quizId: number): Promise<ProcessResult> {
