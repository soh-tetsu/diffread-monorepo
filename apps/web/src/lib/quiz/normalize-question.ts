import type { QuestionContent, QuestionRow } from '@/types/db'
...
function isInstructionQuestionContent(content: QuestionContent): content is Extract<
  QuestionContent,
  {
...
export function normalizeQuestion(row: QuestionRow): QuizQuestion | null {
  ...
  if (content.type === 'mcq') {
    ...
    const answerIdx = content.options.findIndex((opt) => opt.id === content.answer)
