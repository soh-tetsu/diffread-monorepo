...
import { normalizeQuestion, type QuizQuestion } from '@/lib/quiz/normalize-question'
import { supabase } from '@/lib/supabase'
import type { ArticleRow, HookStatus, QuizRow, SessionRow } from '@/types/db'
...
export async function getSessionQuizPayload(sessionToken: string): Promise<SessionQuizPayload> {
  ...
  const [{ data: quiz, error: quizError }, { data: article, error: articleError }] =
    await Promise.all([
      supabase.from('quizzes').select('*').eq('id', session.quiz_id).maybeSingle(),
      supabase.from('articles').select('*').eq('original_url', session.article_url).maybeSingle(),
    ])
  ...
  const [{ data: hookRow, error: hookError }, { data: questionRows, error: questionError }] =
    await Promise.all([
      supabase.from('hook_questions').select('*').eq('quiz_id', quiz.id).maybeSingle(),
      supabase
        .from('questions')
        .select('*')
        .eq('quiz_id', quiz.id)
  ...
  const hookQuestions = hookRow?.hooks ? normalizeHookQuestions(hookRow.hooks) : []
