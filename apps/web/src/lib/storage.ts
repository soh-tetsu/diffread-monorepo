...
const MAX_PDF_SIZE_BYTES = Number(process.env.MAX_PDF_SIZE_BYTES ?? 25 * 1024 * 1024)
...
export async function uploadArticleBundle(
  ...
  const htmlUpload = await supabase.storage.from(ARTICLE_BUCKET).upload(htmlPath, htmlBuffer, {
  ...
  const textUpload = await supabase.storage.from(ARTICLE_BUCKET).upload(textPath, textBuffer, {
...
export async function downloadArticleContent(
  ...
  const normalizedBase = storagePath.endsWith('/') ? storagePath.slice(0, -1) : storagePath
  ...
  const { data, error } = await supabase.storage.from(targetBucket).download(objectPath)
...
export async function uploadArticlePdf(
  ...
  if (nodeBuffer.byteLength > MAX_PDF_SIZE_BYTES) {
    throw new Error(`PDF exceeds limit (${nodeBuffer.byteLength} bytes > ${MAX_PDF_SIZE_BYTES})`)
  }
  ...
  const { error } = await supabase.storage.from(PDF_BUCKET).upload(objectPath, nodeBuffer, {
