...
export async function upsertHookQuestions(
  ...
  if (Object.hasOwn(input, 'hooks')) {
    payload.hooks = input.hooks ?? null
  }
  if (Object.hasOwn(input, 'modelVersion')) {
    payload.model_version = input.modelVersion ?? null
  }
  if (Object.hasOwn(input, 'strategyPrompt')) {
...
export async function getHookQuestionsByQuizId(quizId: number): Promise<HookQuestionRow | null> {
  const { data, error } = await supabase
    .from('hook_questions')
    .select('*')
    .eq('quiz_id', quizId)
