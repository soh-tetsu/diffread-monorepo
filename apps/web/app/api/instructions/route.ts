...
export async function POST(request: Request) {
  try {
    ...
    const result = await enqueueAndProcessSession(session.user_email, articleUrl)
    ...
    const needsReset = new Set<QuizStatus>(['not_required', 'failed'])
    ...
    if (needsReset.has(status)) {
      ...
      if (updateError) {
        throw new Error(`Failed to enqueue instruction workflow: ${updateError.message}`)
      }
