...
export async function POST(request: Request) {
  try {
    ...
    if (!currentToken || !articleUrl) {
      return NextResponse.json({ message: 'Missing currentToken or articleUrl.' }, { status: 400 })
    }
    ...
    const sessionResult = await initSession(currentSession.user_email, articleUrl)
    ...
    const existingHooks = await getHookQuestionsByQuizId(sessionResult.quiz.id)
    ...
    const refreshedHooks = await getHookQuestionsByQuizId(sessionResult.quiz.id)
    ...
    return NextResponse.json({
      ...
      metadata: (articleRecord.metadata?.analysis as Record<string, unknown>) ?? null,
      article: {
        id: articleRecord.id,
