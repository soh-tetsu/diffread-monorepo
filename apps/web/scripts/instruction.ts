...
async function main() {
  const [{ enqueueAndProcessSession }, { processQuizByIdV2: processQuizById }, { supabase }] =
    await Promise.all([
  ...
  const needsReset = new Set(['not_required', 'failed'])

  let instructionStatus: string = status

  if (status === 'ready') {
    logger.info({ quizId: latestQuiz.id, status }, 'Instruction workflow already ready; skipping')
